...
function loadFormData() {
    try {
        ...
        if (formData.schoolEmail) document.getElementById('schoolEmail').value = formData.schoolEmail;
        if (formData.githubUsername) document.getElementById('githubUsername').value = formData.githubUsername;
        if (formData.seniorYear) document.getElementById('seniorYear').value = formData.seniorYear;
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 400) {
                ...
                showAlert('warning', 'Application Submitted',
                ...
                showAlert('error', 'Duplicate Application',
                    result.error || 'An application with this email and GitHub username already exists',
                    '', 10000);
            } else {
...
document.getElementById('applicationForm').addEventListener('submit', async (e) => {
    ...
    if (!validateEmail(email)) {
        showFieldError('schoolEmail', 'Must be a STEM Gharbiya school email (@stemgharbiya.moe.edu.eg)');
        hasErrors = true;
    }

    const seniorYear = formData.get('seniorYear');
    if (!validateSeniorYear(seniorYear)) {
        showFieldError('seniorYear', 'Senior Year must be S25 through S30 only (e.g., S25, S26, S27)');
        hasErrors = true;
    }
...
document.getElementById('seniorYear').addEventListener('blur', function() {
    ...
    if (year && !validateSeniorYear(year)) {
        showFieldError('seniorYear', 'Senior Year must be S25 through S30 only (e.g., S25, S26, S27)');
    }
});
